...
class CIFAR10Oracle(nn.Module):
    def __init__(self, input_size, class_num):
        super(CIFAR10Oracle, self).__init__()
...
def train(model, optimizer, data_loader, writer, model_path, args):
    counter = 0
...
def sub_main():
    ...
    # prepare data
    X = np.load('datasets/cifar10/feature.npy')[-100:]
    y = np.load('datasets/cifar10/label.npy')[-100:]
