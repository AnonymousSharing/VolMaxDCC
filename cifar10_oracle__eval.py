...
def sub_main():
    ...
    X = np.load('datasets/cifar10/feature.npy')
    y = np.load('datasets/cifar10/label.npy')
    ...
    for epoch in range(0, 10, 1): #range(0, 10, 1): #70
        # initialize model
        model = CIFAR10Oracle(512, class_num)
        trained_model_path = os.path.join('save/cifar10-oracle/', "checkpoint_{}.tar".format(epoch))
